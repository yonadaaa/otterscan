...
const unsupportedDynamicField = (fieldType: SchemaType): never => {
  throw new Error(
    `Unsupported dynamic field type: ${SchemaType[fieldType] ?? fieldType}`
  );
};

// TODO: figure out how to use with SchemaTypeToPrimitiveType<T> return type to ensure correctness here
export const decodeDynamicField = <T extends DynamicSchemaType>(
  fieldType: T,
  bytes: Uint8Array
) => {
  ...
  if (staticType !== undefined) {
    ...
    return new Array(arrayLength)
      .fill(undefined)
      .map((_, i) => decodeStaticField(staticType, bytes, i * fieldLength));
  }
