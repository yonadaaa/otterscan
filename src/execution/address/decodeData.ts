...
export const decodeData = (
  ...
  if (schema.dynamicFields.length > 0) {
    const dynamicDataLayout = bytes.slice(
      schema.staticDataLength,
      schema.staticDataLength + 32
    );
    bytesOffset += 32;
    ...
    schema.dynamicFields.forEach((fieldType, i) => {
      const dataLength = decodeStaticField(
        ...
        dynamicDataLayout,
        getStaticByteLength(packedCounterAccumulatorType) +
          i * getStaticByteLength(packedCounterCounterType)
      ) as number;
