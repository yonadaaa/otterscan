import { TableSchema } from "@latticexyz/network/src/v2/common";
import {
  getStaticByteLength,
  SchemaType,
  SchemaTypeToAbiType,
...
export function decodeSchema(rawSchema: string): TableSchema {
  ...
  const buffer = isEmpty
    ? new Uint8Array(64).buffer
    : hexToArray(rawSchema).buffer;
  ...
  const valueSchema = {
    ...decodeSchemaBytes(valueSchemaBytes),
    rawSchema,
    isEmpty,
  };
  const keySchema = {
    ...decodeSchemaBytes(keySchemaBytes),
    rawSchema,
    isEmpty,
  };
...
function decodeSchemaBytes(schemaBytes: DataView) {
  ...
  for (
    let i = 4 + numStaticFields;
    i < 4 + numStaticFields + numDynamicFields;
    i++
  ) {
  ...
  const actualStaticDataLength = staticFields.reduce(
    (acc, fieldType) => acc + getStaticByteLength(fieldType),
    0
  );
  ...
  const abiTypes = [...staticFields, ...dynamicFields].map(
    (type) => SchemaTypeToAbiType[type]
  );
